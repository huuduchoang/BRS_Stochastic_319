...
%
% State:
%   u = [v; n0; n1; n2; n3; n4; h; alpha; vollung; PO2lung; PO2blood]

    t  = (0:dt:tf).';
    nt = length(t);

    % number of K channels, matching HHSS14D convention
    NK = round(18*Area);
    ...
    for k = 1:nt-1
        ...
        %------------------------------------------
        % x = [v; h; alpha; vollung; PO2lung; PO2blood]
        x0 = uk([1 7 8 9 10 11]);
        ...
        u_new(7)    = x_new(2);   % h
        u_new(8)    = x_new(3);   % alpha
        u_new(9)    = x_new(4);   % vollung
        u_new(10)   = x_new(5);   % PO2lung
        u_new(11)   = x_new(6);   % PO2blood
